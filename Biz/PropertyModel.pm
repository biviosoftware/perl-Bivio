...
Calls L<parse_query_from_request|"parse_query_from_request">
and then L<load|"load">.

See also L<unsafe_load_from_request|"unsafe_load_from_request">.
...
=head2 parse_query_from_request() : hash_ref

Parses the load args from the query string generated by
L<format_query|"format_query"> or by
L<Bivio::SQL::ListQuery|Bivio::SQL::ListQuery>.

If the query has a problem, C<undef> will be returned and a
warning will be output.  If there is no query, no warning will
be output.
...
sub parse_query_from_request {
	...
	return _query_err($self, 'missing this') unless $parent_id;
...
=for html <a name="unsafe_load_from_request"></a>

=head2 unsafe_load_from_request() : boolean

Calls L<parse_query_from_request|"parse_query_from_request">
and then L<unsafe_load|"unsafe_load">.  If there is no query
or the query is corrupt, returns false.

See also L<load_from_request|"load_from_request">.

=cut

sub unsafe_load_from_request {
    my($self) = @_;
    my($q) = $self->parse_query_from_request();
...
sub _query_err {
    my($self, $msg) = @_;
    $self->get_request->warn($self, '->unsafe_load_from_request: ', $msg);
    return undef;
}
