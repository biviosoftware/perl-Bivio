...
sub format_query {
    ...
    my($pk_cols, $version) = $support->get('primary_key', 'version');
    my($properties) = $self->internal_get;
    my($res) = '';
    # NOTE: Nice to agree with ListQuery::_format_uri_primary_key
    foreach my $col (@$pk_cols) {
	$res .= $col->{type}->to_uri($properties->{$col->{name}})."\177";
    }
    chop($res);
    return 'v='.$version.'&t='.$res;
}
...
sub load_from_query {
    ...
    if (ref($q) eq 'HASH') {
	my($support) = $self->internal_get_sql_support;
	my($version, $pk_cols) = $support->get('version', 'primary_key');
	if (defined($q->{v}) && $q->{v} == $version) {
	    if (defined($q->{t})) {
		my(@query) = ();
		my(@pk) = split(/\177/, $q->{t});
		foreach my $col (@$pk_cols) {
#TODO: Need to check for correct number of \177 values
		    my($literal) = shift(@pk);
		    my($v, $err) = $col->{type}->from_literal($literal);
		    $self->die(Bivio::DieCode::CORRUPT_QUERY(),
			    {column => $col->{name}, literal => $literal,
				error => $err ? $err->as_string : 'NULL'})
			    unless defined($v);
		    push(@query, $col->{name}, $v);
		}
		# Success with parsing, let's see if it is there
		return $self->load(@query);
	    }
	    else {
		_trace('missing this in query') if $_TRACE;
	    }
	}
	else {
	    _trace('invalid version: ', $q->{v}, ' != ', $version) if $_TRACE;
	}
    }
    else {
	_trace('no query string') if $_TRACE;
    }
    $self->die(Bivio::DieCode::CORRUPT_QUERY());
}
